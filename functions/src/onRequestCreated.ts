...
const APP_BASE_URL = defineSecret("APP_BASE_URL"); // เช่น https://imperialworld.asia
...
const fmtDate = (dt: Date | null): string => {
  if (!dt) return "-";
  try {
...
const durationText = (from: Date | null, to: Date | null): string => {
  if (!from || !to) return "-";
...
function uniqEmails(arr: (string | undefined | null)[]): string[] {
...
async function getAdminRecipients(): Promise<string[]> {
  ...
  snap.forEach((doc) => {
    ...
    const caps: string[] = Array.isArray(data.caps) ? data.caps : [];
    ...
    if ((isAdminRole || canApprove) && email) emails.push(email);
  });
...
export const onRequestCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    const rid = event.params.rid as string;
    const rec = (snap.data() as any) || {};

    // ---------- STEP 1: transaction lock (กันยิงซ้ำ) ----------
    const reqRef = db.collection("requests").doc(rid);
    const locked = await db.runTransaction(async (tx) => {
      const doc = await tx.get(reqRef);
      const mailLog = (doc.get("mailLog") as any) || {};
      const hasSent = !!mailLog?.createdNoticeSentAt;
      const hasLock = !!mailLog?.createdNoticeLock;
      if (hasSent || hasLock) return true;
      tx.set(
        reqRef,
        { mailLog: { createdNoticeLock: FieldValue.serverTimestamp() } },
        { merge: true }
      );
      return false;
    });
    ...
    const workers = (rec?.work?.people ?? rec?.people ?? rec?.workers ?? []) as any[];
    ...
    const firstWorkers = (Array.isArray(workers) ? workers : [])
      .slice(0, 6)
      .map((w: any) => w?.name || w?.fullname || `${w?.firstname || ""} ${w?.lastname || ""}`.trim())
      .filter(Boolean);

    const supervisor =
      rec?.work?.supervisor?.name ?? rec?.supervisor?.name ?? rec?.supervisor ?? "";
    ...
    const qrPng = await QRCode.toBuffer(statusLink, { type: "png", width: 200 });

    // รายชื่อผู้รับ (admin + approver) → BCC
    const recipients = await getAdminRecipients();
    const bccList = uniqEmails(recipients);
    if (bccList.length === 0) bccList.push((SMTP_FROM.value() || "").toString());
    ...
    if (requesterEmail) {
      ...
      const bulletproofBtnRequester = (href: string, label: string) => `
        <!--[if mso]>
...
`;
    ...
    const bulletproofBtnAdmin = (href: string, label: string) => `
      <!--[if mso]>
    ...
    await reqRef.set(
      {
        mailLog: {
          ...
          createdNoticeLock: FieldValue.delete(),
          lastMessageId: (info as any)?.messageId || null,
