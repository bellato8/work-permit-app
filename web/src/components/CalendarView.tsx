...
//  - ✅ ใช้ GridLegacy (alias เป็น Grid) เพื่อให้โค้ดเดิม build ผ่าน โดยไม่แตะโครงหน้าจอ
// ======================================================================
...
import {
  ...
  Alert,
  Button,
  // ใช้ GridLegacy แต่ alias เป็น Grid เพื่อให้ JSX เดิมใช้ต่อได้
  GridLegacy as Grid,
} from "@mui/material";
...
// 0 = อาทิตย์ขึ้นก่อน, 1 = จันทร์ขึ้นก่อน
...
// คืน "YYYY-MM-DD" ด้วยเวลาท้องถิ่น
...
export default function CalendarView({ onDateClick }: CalendarViewProps) {
  ...
  async function loadCalendarData() {
    ...
    try {
      const apiMonth = month + 1; // JS 0-11 → API 1-12
  ...
  // ========== สร้างกริดของเดือน ==========
  const firstDayRaw = new Date(year, month, 1).getDay(); // 0=อา..6=ส (ตามเวลาท้องถิ่น) :contentReference[oaicite:4]{index=4}
  const firstDayIndex =
  ...
  if (error) {
    return (
      <Alert severity="error" sx={{ mb: 2 }}>
        {error}
        <Button size="small" onClick={loadCalendarData} sx={{ ml: 2 }}>
          ลองอีกครั้ง
        </Button>
      </Alert>
    );
