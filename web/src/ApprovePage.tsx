...
const UPDATE_URL =
  ...
  "https://updatestatus-aa5gfxjdmq-as.a.run.app"; // เปลี่ยนเป็นของโปรเจ็กต์คุณถ้าต่าง
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const r = await fetch(url, {
        ...
        // ฟังก์ชันฝั่งเซิร์ฟเวอร์คาดหวัง action, key, reason
      ...
      <div className="w-full max-w-2xl bg-white/90 border border-slate-200 rounded-2xl p-5 shadow-lg">
        ...
        <form className="grid gap-4" onSubmit={onSubmit}>
          <div>
            ...
            <input className="input" value={rid} onChange={(e) => setRid(e.target.value.trim())} placeholder="WP-YYYYMMDD-XXXX" />
          </div>

          <div>
            <label className="label">Approver Key</label>
            <input className="input" value={key} onChange={(e) => setKey(e.target.value.trim())} placeholder="คีย์สำหรับผู้อนุมัติ" />
            <p className="text-xs text-slate-500 mt-1">ระบบจะเติมให้อัตโนมัติถ้าเข้าเพจจากลิงก์ในอีเมล</p>
          </div>
        ...
        {/* ผลลัพธ์ */}
        {resp && (
          <div className={"mt-4 rounded-xl p-3 text-sm " + (resp.ok ? "border-green-200 bg-green-50 text-green-800" : "border-rose-200 bg-rose-50 text-rose-800")}>
            {resp.ok ? (
              <>
                บันทึกสำเร็จ •{" "}
                <a className="underline" href={`/status?rid=${encodeURIComponent(rid)}`} target="_blank">
